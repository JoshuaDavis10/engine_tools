...
unsigned char initialize_events() {
    if(state_ptr != 0) {
        LOGWARN("Tried to initialize event system state after it was already initialized");
        return 0;
    }
...
void shutdown_events() {
    ...
    free(state_ptr);
    state_ptr = 0;
    LOGINFO("Event system state shut down");
}
...
unsigned char register_event (int code, void* listener, event_callback callback) {
    ...
    for(int i = 0 ; i < numRegistered ; i++) {
        if(state_ptr->registered[code].events[i].listener == listener) {
            LOGWARN("Tried to register an event that has already been registered");
            return 0;
        }
    }
